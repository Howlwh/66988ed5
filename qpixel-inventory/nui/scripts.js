...
$(document).ready(function() {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://qpixel-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.response == 'openGui') {
            ...
            if (itemList[item.id]) {
                $.post(
                    'https://qpixel-inventory/GiveItem',
                    JSON.stringify([
                        item.id,
                        item.amount,
...
function invStack(
    ...
    $.post('https://qpixel-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://qpixel-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://qpixel-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount, CraftArrayID) {
    let arr = itemList[itemid].craft[CraftArrayID];
    let amount = moveAmount;
    $.post('https://qpixel-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0, targetInvSlots = 40) {
    ...
    } else if (targetinvName.indexOf('recycle') > -1 || targetinvName.startsWith('restaurants_elf')) {
        secondaryMaxWeight = 300.0;
        slotLimitTarget = 40;
        displayName = 'Food Shelf'
    ...
    } else if (targetinvName.startsWith('dodopackagesmall')) {
        secondaryMaxWeight = 50.0;
        slotLimitTarget = 5;
        displayName = 'Dodo Package';
    } else if (targetinvName.startsWith('dodopackagemedium')) {
        secondaryMaxWeight = 85.0;
        slotLimitTarget = 25;
        displayName = 'Dodo Package';
    } else if (targetinvName.startsWith('dodopackagelarge')) {
        secondaryMaxWeight = 120.0;
        slotLimitTarget = 75;
        displayName = 'Dodo Package';
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://qpixel-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://qpixel-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://qpixel-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://qpixel-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://qpixel-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://qpixel-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://qpixel-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://qpixel-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://qpixel-inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://qpixel-inventory/craftProgression', JSON.stringify({
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://qpixel-inventory/invuse', JSON.stringify(arr));
