...
local function processStressBlock()
    TriggerEvent("qpixel-buffs:applyBuff", "joints", { { buff = "stressblock", percent = 1.0 } })
end
...
AddEventHandler("qpixel-katana:removeEquippedKatana", function()
...
RegisterCommand('+useKeyFob', function()
    if Throttled("qpixel-doorss:doorKeyFob", 1000) then return end
    TriggerEvent("qpixel-doors:doorKeyFob")
end, false)
RegisterCommand('-useKeyFob', function() end, false)

Citizen.CreateThread(function()
    exports["qpixel-keybinds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (isWeapon) then
        ...
        Wait(1500)
        TriggerEvent("AttachWeapons")
        TriggerEvent('qpixel-hud:armed', 100)
        return
    end

    if itemid == "murdermeal" then
        local finished = exports['qpixel-taskbar']:taskBar(1000, 'Unwrapping')
        local cid = exports["isPed"]:isPed("cid")
    ...
    if itemid == "dodopackagesmall" then
        local finished = exports['qpixel-taskbar']:taskBar(1000, 'Unwrapping')
        local cid = exports["isPed"]:isPed("cid")
        TriggerEvent("InteractSound_CL:PlayOnOne","unwrap",1.0)
        if finished == 100 then
            TriggerEvent("server-inventory-open", "1", "dodopackagesmall-"..ItemInfo.id)
        end
    end

    if itemid == "dodopackagemedium" then
        local finished = exports['qpixel-taskbar']:taskBar(1000, 'Unwrapping')
        local cid = exports["isPed"]:isPed("cid")
        TriggerEvent("InteractSound_CL:PlayOnOne","unwrap",1.0)
        if finished == 100 then
            TriggerEvent("server-inventory-open", "1", "dodopackagemedium-"..ItemInfo.id)
        end
    end

    if itemid == "dodopackagelarge" then
        local finished = exports['qpixel-taskbar']:taskBar(1000, 'Unwrapping')
        local cid = exports["isPed"]:isPed("cid")
        TriggerEvent("InteractSound_CL:PlayOnOne","unwrap",1.0)
        if finished == 100 then
            TriggerEvent("server-inventory-open", "1", "dodopackagelarge-"..ItemInfo.id)
        end
    end
    ...
    if itemid == "bstoy" then
        TriggerEvent('qpixel-burgershot:gettoy')
        ...
        if GetEntityModel(PlayerPedId()) == `Mr_kebun` then
          if tonumber(ItemInfo.quality) > 0 then
              if not isKatanaEquipped then
                ...
                Citizen.CreateThread(function()
                    Citizen.Wait(1000)
                    TriggerServerEvent("qpixel-katana:cursedKatanaEquip", GetEntityCoords(PlayerPedId()))
                    TriggerEvent("qpixel-katana:cursedKatanaEquipC")
                end)
              else
                isKatanaEquipped = false
              end
    ...
    if stolenItems[itemid] and exports["qpixel-npcs"]:isCloseToPawnPed() then
    ...
    if itemid == "key1" then
        TriggerEvent('qpixel-doors:unlockContainer1')
    end

    if itemid == "key2" then
        TriggerEvent('qpixel-doors:unlockContainer2')
    end

    if (itemid == "pdbadge") then
        TriggerEvent("qpixel-policeBadge:showBadge")
    end
    ...
    if (itemid == "heistlaptop3") then
        TriggerEvent('qpixel-robbery:usb')
    end 

    if (itemid == "heistlaptop2") then
        TriggerEvent('qpixel-paleto:UseBlueLapTop')
    end
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["qpixel-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "femaleseed") then
        ...
        TriggerEvent("qpixel-weed:plantSeed", itemid)
     end
    ...
    if ( itemid == "smallbud" and hasEnoughOfItem("qualityscales",1,false) ) then
        local finished = exports["qpixel-taskbar"]:taskBar(1000,"Packing Joint",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weedq") then
        local finished = exports["qpixel-taskbar"]:taskBar(1000,"Rolling Joints",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "qtakis") then
        ...
        TriggerEvent('qpixel-hud:ChangeHunger', 30)
    end
    ...
    if (itemid == "qdrpepper") then
        ...
        TriggerEvent('qpixel-hud:ChangeThirst', 30)
    end
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["qpixel-taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint" or itemid == "joint2" or itemid == "customjointitem") then
        local finished = exports["qpixel-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)
        if (finished == 100) then
            remove = true
            smoking = true
    ...
    if itemid == "clotion" or itemid == "cabsinthe" then
        remove = true
        local finished = exports['qpixel-taskbar']:taskBar(2500, 'Consuming Lotion')
        if finished == 100 then
            SetPlayerMaxArmour(PlayerPedId(), 100)
    ...
    if (itemid == "heavyammo") then
        local finished = exports["qpixel-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pistolammo") then
        local finished = exports["qpixel-taskbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "flamethrowerammo") then
        local finished = exports["qpixel-taskbar"]:taskBar(20000,"Reloading flamethrower",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "huntingammo") then
        local finished = exports["qpixel-taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "rpgammo") then
        local finished = exports["qpixel-taskbar"]:taskBar(25000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "taserammo") then
        local finished = exports["qpixel-taskbar"]:taskBar(2000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "empammo") then
        local finished = exports["qpixel-taskbar"]:taskBar(30000,"Recharging EMP",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "armor" or itemid == "pdarmor") then
        local finished = exports["qpixel-taskbar"]:taskBar(4500,"Putting on Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "cbrownie" or itemid == "cgummies") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["qpixel-taskbar"]:taskBar(3000,"Consuming edibles 😉",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["qpixel-taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["qpixel-taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "fishingtacklebox") then
        local finished = exports["qpixel-taskbar"]:taskBar(5000,"Opening",true,false,playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "housesafe") then
        if not hasEnoughOfItem("heavydutydrill", 1, false) then
            ...
            local finished = exports["qpixel-taskbar"]:taskBar(30000, "Cracking Safe...", true, false)
            if (finished == 100) then
                remove = true
                TriggerServerEvent('loot:useItem', itemid)
            end
    ...
    if (itemid == "fishingchest") then
        local finished = exports["qpixel-taskbar"]:taskBar(5000, "Opening Chest", true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "repcrate") then
        local finished = exports["qpixel-taskbar"]:taskBar(30000, "Opening...", true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "babyoil") then
        local finished = exports["qpixel-taskbar"]:taskBar(10000,"Applying baby oil to dome",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = exports["qpixel-taskbar"]:taskBar(5000,"Opening cooler",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "pixellaptop")  then
        TriggerEvent("qpixel-boosting:open:boostingApp")
    end    
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["qpixel-taskbar"]:taskBar(10000,"Opening bank box.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["qpixel-taskbar"]:taskBar(5000,"Opening briefcase.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["qpixel-taskbar"]:taskBar(15000,"Opening supply crate.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "xmasgiftcoal") then
        local finished = exports["qpixel-taskbar"]:taskBar(15000, "Opening Gift", false, false, playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "emptyballoon") then
        if (hasEnoughOfItem("nitrous",1,false)) then
            local finished = exports["qpixel-taskbar"]:taskBar(5000,"Filling Balloon.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Peecup_empty") then
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(10000, "Peeing in cup.", false, false, playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "megaphone") then
      TriggerEvent("qpixel-usableprops:megaphone")
    end
            ...
            Citizen.CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["qpixel-taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
            finished = exports["qpixel-taskbar"]:taskBar(20000,"Nitrous")
            if (finished == 100 and not cancelNos) then
                TriggerServerEvent('qpixel-vehicles:ApplyNitrous')
                remove = true
            else
    ...
    if (itemid == "ciggy") then
        local finished = exports["qpixel-taskbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "oxygentank") then
        local finished = exports["qpixel-taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then
    ...
    if itemid == "rice_balls" then
        ...
        local finished = exports['qpixel-taskbar']:taskBar(2500, 'Eating...')
        if finished == 100 then
            TriggerEvent('qpixel-hud:ChangeHunger', 50)
    ...
    if itemid == "chicken_noodle_soup" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Slurping Soup...')
        if finished == 100 then
            TriggerEvent('qpixel-hud:ChangeHunger', 80)
    ...
    if itemid == "doki_doki_pancakes" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Eating Pancakes...')
        if finished == 100 then
            TriggerEvent('qpixel-hud:ChangeHunger', 50)
    ...
    if itemid == "chocolate_cake" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Eating Cake...')
        if finished == 100 then
            TriggerEvent('qpixel-hud:ChangeHunger', 25)
    ...
    if itemid == "strawberry_shortcake" then
        ...
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Eating Cake...')
        if finished == 100 then
            TriggerEvent('qpixel-hud:ChangeHunger', 45)
    ...
    if (itemid == "softdrink") then
        ...
        TriggerEvent('qpixel-hud:ChangeThirst', 69)
        TriggerEvent('thirstbuff')
    end
    ...
    if itemid == "torpedo" then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('qpixel-hud:ChangeHunger', 15)
        TriggerEvent('aspect-buffs:bulb_buff')
    end
    ...
    if itemid == "moneyshot" then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('qpixel-hud:ChangeHunger', 69)
        TriggerEvent('aspect-buffs:money_boost_buff')
    end
    ...
    if (itemid == "pepperonipizza") then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('qpixel-hud:ChangeHunger', 60)
        TriggerEvent('aspect-buffs:money_boost_buff')
    end
    ...
    or itemid == "experiment_09") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["qpixel-taskbar"]:taskBar(3000,"Downing content of bottle. 💊",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pillbox") then
        local finished = exports["qpixel-taskbar"]:taskBar(5000, "Opening pill box", false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "skateboard") then
        TriggerEvent('qpixel-longboard:PlaceDown')
    end
    ...
    if itemid == "customfooditem" then
        TriggerEvent("qpixel-hud:ChangeHunger", 100)
    end
    ...
    if itemid == "kdragonwater" then
        ...
        AttachPropAndPlayAnimation(
          ...
          6000,
          "Drink",
          "qpixel-katana:transformKFromDrink",
          true,
          itemid,
          playerVeh
        )
    ...
    if (itemid == "methlabproduct") then
        local finished = exports["qpixel-ui"]:taskBarSkill(2400, math.random(12, 18))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if itemid == "ketamine" then
        local finished = exports["qpixel-ui"]:taskBarSkill(2000, math.random(10, 15))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim","drugpackage01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["qpixel-ui"]:taskBarSkill(1000, math.random(7, 12))
        if (finished == 100) then
    ...
    if itemid == "roostertea" then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,15000,"Drink","qpixel-roostersrest:drinkTea",true,itemid,playerVeh)
    end
    ...
    if itemid == "franksflute" then
      TriggerServerEvent("qpixel-inventory:franksFlute", GetEntityCoords(PlayerPedId()))
    ...
    if itemid == "tattooremover" then
        local target = exports['qpixel-target']:GetCurrentEntity()
        --local target = PlayerPedId()
        if IsEntityAPed(target) and IsPedAPlayer(target) then
            TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_WELDING", 0, true)
            local finished = exports["qpixel-taskbar"]:taskBar(60000, "Removing tattoos")
    ...
    if itemid == "buddhaletter" then
        if GetEntityModel(PlayerPedId()) == `ig_buddha` or GetEntityModel(PlayerPedId()) == `g_m_m_buddha` then
            local finished = exports["qpixel-taskbar"]:taskBar(5000,"Opening letter.",false,false,playerVeh)
            if finished == 100 then
        ...
        if hasEnoughOfItem("buddhablade",1,false) and hasEnoughOfItem("buddhaguard",1,false) and hasEnoughOfItem("buddhahilt",1,false) then
            if hasEnoughOfItem("buddhamedalion",1,false) then
                ...
                if #(playerPos - vector3(-172.54, 319.13, 87.35)) < 10 then
                    local finished = exports["qpixel-taskbar"]:taskBar(30000,"Putting sword together.",false,false,playerVeh)
                    if (finished == 100) then
    ...
    if itemid == "buddhashovel" then
        ...
        if #(playerPos - vector3(-1911.78, 1388.1, 219.35)) < 10 then
            if BuddhaIsItemFound1 == false then
                ...
                local finished = exports["qpixel-taskbar"]:taskBar(15000, "Digging for something.", false, true, false, false, nil, 5.0, PlayerPedId())
                ClearPedTasks(PlayerPedId())
                if finished == 100 then
    ...
    TriggerEvent("qpixel-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
    TriggerServerEvent("qpixel-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
...
AddEventHandler("qpixel-inventory:attachPropPlayAnim", function(pType)
    ...
    TriggerEvent("qpixel-buffs:itemUsedSuccess", success)
end)
...
RegisterNetEvent('qpixel-business:repairItem')
AddEventHandler('qpixel-business:repairItem', function()
    
    local info = GetItemInfo(1)
    local Quality = info.quality
    local QualityCheck = (Quality < 80 and Quality or false)
    local PercentRepair = 80 - Quality
    local itemListInfo = json.decode(exports["qpixel-inventory"]:itemListInfo(info.item_id))
    local allowed = true  
...
RegisterNetEvent('qpixel-business:examineItem')
AddEventHandler('qpixel-business:examineItem', function()
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid,quality)
    ...
    if timer > 0 then
        local finished = exports["qpixel-taskbar"]:taskBar(timer,message,true,false,playerVeh)
...
function checkForItems()
    ...
    for _, item in ipairs(items) do
        ...
        if hasItem and not PreviousItemCheck[item] then
            ...
            TriggerEvent('qpixel-inventory:itemCheck', item, true, quantity)
        elseif not hasItem and PreviousItemCheck[item] then
            PreviousItemCheck[item] = false
            TriggerEvent('qpixel-inventory:itemCheck', item, false, quantity)
        end
    end
end
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["qpixel-ui"]:taskBarSkill(15000,3)

            if finished ~= 100 then
                 lockpicking = false
                return
            end
            exports['qpixel-dispatch']:dispatchadd('10-99B', 'Stolen Vehicle', '227')

            local finished = exports["qpixel-ui"]:taskBarSkill(2200,4)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["qpixel-ui"]:taskBarSkill(math.random(5000,10000),math.random(10,15))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            
            TriggerEvent('qpixel-dispatch:stolenCar', GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)))
            ...
            local finished = exports["qpixel-ui"]:taskBarSkill(1500,math.random(5,10))
            if finished ~= 100 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["qpixel-ui"]:taskBarSkill(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["qpixel-ui"]:taskBarSkill(timer,math.random(4,7))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
function getInformationFromFirstItem(itemToFind)
  local foundItem = exports["qpixel-inventory"]:GetInfoForFirstItemOfName(itemToFind)
...
RegisterInterfaceCallback("qpixel-ui:inv:moneycaseAction", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
  exports['qpixel-ui']:closeApplication('textbox')
  RPC.execute("qpixel-inventory:doMoneycaseAction", data)
end)

AddEventHandler("qpixel-inventory:openGolfStore", function(name, info)
...
function openBurner(info)
    ...
    exports["qpixel-ui"]:openApplication("burner", info)
end
...
AddEventHandler("ammonium_bicarbonate:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(5000 * quantity, "Breaking down into baggies...", false, false)
...
function notepadUsed()
    exports["qpixel-ui"]:openApplication("notepad", {
        canSave = true
    })
    TriggerEvent("animation:PlayAnimation", "notepad")
end

RegisterInterfaceCallback('qpixel-ui:cancelNotepadEmote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent("animation:PlayAnimation", "cancel")
end)

RegisterInterfaceCallback('qpixel-ui:createNotepadNote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    exports["qpixel-ui"]:closeApplication("notepad")
